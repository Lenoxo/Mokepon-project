...
const contenedorTarjetasMokepon = document.getElementById(
  "contenedor-tarjetas-mascotas"
);
...
function unirseAlJuego() {
  ...
  fetch("http://192.168.0.13:8080/unirse").then(function (res) {
    if (res.ok) {
...
function enviarAtaques() {
  ...
  // Esta linea causó dolores de cabeza, recuerda que al usar setInterval, no es necesario llamar la función, solo usar el nombre basta.
  intervalo = setInterval(obtenerAtaques, 50);
}
function obtenerAtaques() {
  fetch(`http://192.168.0.13:8080/mokepon/${enemigoId}/ataques`).then(function (
    res
  ) {
    if (res.ok) {
...
function crearMensaje() {
  ...
  if (victoriasJugador === victoriasEnemigo) {
    ...
    sectionResultado.innerHTML =
      "Ha finalizado la partida, has ganado la batalla 😎😎😎";
    mostrarSectionReiniciar();
  } else {
    sectionResultado.innerHTML =
      "Ha finalizado la partida, has perdido la batalla 😣😣😣";
    mostrarSectionReiniciar();
  }
}
